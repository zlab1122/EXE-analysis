...
met_target <- as.data.frame(t(read.csv("blood_metabolitis2.csv", row.names = 1, check.names = FALSE)[target_mets, ]), check.names = FALSE) %>% mutate(Sample_ID = rownames(.))
...
for (var in target_mets) {
  if(var %in% colnames(df_all)) {
    df_all[[paste0("z_", var)]] <- as.numeric(df_all[[var]])
  }
}
...
out_dir <- "Clin_Met_Plots_Mixed_24_0513_Rev"
